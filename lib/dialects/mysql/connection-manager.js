...
var AbstractConnectionManager = require('../abstract/connection-manager')
  ...
  , timezoneParser = require('../../utils/mysql-timezone-parser')
  , parserMap = {};
...
ConnectionManager.prototype.connect = function(config) {
  ...
  return new Promise(function (resolve, reject) {
    var connectionConfig = {
      ...
      dateStrings: self.sequelize.options.dateStrings,
    ...
    connection.connect(function(err) {
      ...
      var timezone = timezoneParser(self.sequelize.options.timezone);
      connection.query('SET time_zone = "' + timezone + '"', function(err) {
        if (err) {
          reject(new Error('query error: timezone has no data for ' + timezone));
        } else {
          resolve(connection);
        }
      });
